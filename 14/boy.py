...
class WalkingState:
    ...
    def do(boy):
        ...
        boy.x = clamp(25, boy.x, server.background.w - 26)
        boy.y = clamp(35, boy.y, server.background.h - 46)

    def draw(boy):

        if boy.x_velocity > 0:
            boy.image.clip_draw(int(boy.frame) * 100, 100, 100, 100, boy.sx, boy.sy)
            boy.dir = 1
        elif boy.x_velocity < 0:
            boy.image.clip_draw(int(boy.frame) * 100, 0, 100, 100, boy.sx, boy.sy)
            boy.dir = -1
        else:
            # if boy x_velocity == 0
            if boy.y_velocity > 0 or boy.y_velocity < 0:
                ...
                # boy is idle
                if boy.dir == 1:
                    boy.image.clip_draw(int(boy.frame) * 100, 300, 100, 100, boy.sx, boy.sy)
                else:
                    boy.image.clip_draw(int(boy.frame) * 100, 200, 100, 100, boy.sx, boy.sy)
...
class Boy:
    ...
    def get_bb(self):
        return self.sx - 15, self.sy - 45, self.sx + 15, self.sy + 45
    ...
    def draw(self):
        ...
        self.font.draw(self.sx - 45, self.sy + 75, '(%d, %d)' % (self.x, self.y), (255, 255, 0))
        self.font.draw(self.sx - 12, self.sy + 55, '(%d)' % self.score, (255, 0, 0))
